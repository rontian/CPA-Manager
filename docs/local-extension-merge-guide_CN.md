...
- 内置角色预设中的推荐模型仅用于 CPA-Manager 展示和人工参考，不写入 CLIProxyAPI `auto-router` schema。
- Auto Router 的策略字段写入 `models[].policy.strategy`，角色候选池写入 `roles[].candidates`；候选池为空时仍由旧的 `roles[].provider/model` 生效。
- 角色成本等级 UI 只允许选择 `low`、`medium`、`high`，界面文案可本地化，但落盘值必须保持英文枚举。
- 模型能力清单属于人工维护的前端参考数据，存放在 `src/features/modelCatalog/modelCatalog.ts`，不写入 CLIProxyAPI 配置、不调用管理 API。
- 模型能力标签要同时覆盖任务角色、模型能力、模态能力和运营选择，例如代码、调试、架构、规划、审查、推理、数学、数据分析、工具调用、智能体、图像理解、图像生成、长上下文、快速、性价比、旗舰、中文优化。
